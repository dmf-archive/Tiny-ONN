...
@pytest.mark.skip(
    reason="Skipping due to persistent, unidentified CUDA errors in backward pass."
)
def test_surgery_preserves_gradient_flow():
        ...
        for expert_idx, expert in enumerate(moe_layer.experts):
            for name, param in expert.named_parameters():
                ...
                if param.grad is not None:
                    print(
                        f"  - Grad found for layer {layer.layer_idx}, expert {expert_idx}, param {name}"
                    )
                    grads_found += 1
                else:
                    print(
                        f"  - Grad MISSING for layer {layer.layer_idx}, expert {expert_idx}, param {name}"
                    )

    assert grads_found > 0, (
        "No gradients were found in any expert parameters after surgery!"
    )
    print(
    ...
    assert (
        grads_found >= model.config.num_experts_per_tok * model.config.num_hidden_layers
