...
class ExpertGroup(nn.Module):
    def __init__(self, d_model: int, n_experts_per_group: int, top_k_l2: int):
        ...
        self.experts = nn.ModuleList([Expert(d_model, d_model * 2) for _ in range(self.n_experts)])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        routing_weights = nn.functional.softmax(routing_weights, dim=-1, dtype=torch.float).to(x.dtype)

        final_output = torch.zeros_like(x)
        
        flat_expert_indices = selected_experts.flatten()
        flat_routing_weights = routing_weights.flatten()
        token_indices = torch.arange(x.size(0), device=x.device).repeat_interleave(self.top_k)
...
class HierarchicalMoE(nn.Module):
    def __init__(self, d_model: int, n_groups: int, n_experts_per_group: int, top_k_l1: int, top_k_l2: int):
        super().__init__()
        self.d_model = d_model
        self.n_groups = n_groups
        self.top_k = top_k_l1
        self.gate = nn.Linear(d_model, self.n_groups, bias=False)
        self.groups = nn.ModuleList([ExpertGroup(d_model, n_experts_per_group, top_k_l2) for _ in range(self.n_groups)])
...
class PocHMoEModel(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, n_groups: int, n_experts_per_group: int, top_k_l1: int, top_k_l2: int):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, d_model)
        self.hmoe = HierarchicalMoE(d_model, n_groups, n_experts_per_group, top_k_l1, top_k_l2)
