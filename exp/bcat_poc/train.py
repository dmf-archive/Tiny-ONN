...
def generate_arithmetic_data(num_samples, seq_len, operand_ranges):
    ...
    def str_to_tokens(s):
        return [TOKENIZER.get(c) for c in str(s) if c in TOKENIZER]
    ...
    while len(sents) < num_samples:
        a = random.randint(*operand_ranges)
        b = random.randint(*operand_ranges)
        res = a + b
        problem_tokens = str_to_tokens(a) + [TOKENIZER['+']] + str_to_tokens(b)
...
def run_training_loop():
    model = ReferenceModel().to(device=CONFIG["DEVICE"])
