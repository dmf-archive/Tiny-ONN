...
from optimizers.F3EO import F3EO
...
class TrainingAgent:
    ...
    def run_epochs(self, trial: int, epochs: List[int]) -> None:
        for epoch in epochs:
            ...
            train_loss, train_acc1, train_acc5, grad_norm = self.epoch_iteration(epoch)
            ...
            results = {"train_loss": train_loss, "train_acc1": train_acc1, "train_acc5": train_acc5,
                       ...
                       "mem_used": mem_used, "grad_norm": grad_norm}
        ...
        for i, (inputs, targets) in enumerate(tqdm(self.train_loader, desc=f"Epoch {epoch}")):
            ...
            if self.config['optimizer'] in ["Shampoo", "kfac"]:
                ...
                if self.autocast:
                    ...
                    outputs = self.network(inputs)
                    loss = self.criterion(outputs, targets)
                if isinstance(self.optimizer, Adahessian) or isinstance(self.optimizer, F3EO):
            ...
            if isinstance(self.optimizer, F3EO):
                grad_norms.update(self.optimizer.grad_norm, inputs.size(0))
            if isinstance(self.scheduler, OneCycleLR):
                self.scheduler.step()
        return losses.avg, top1.avg, top5.avg, grad_norms.avg
...
def setup_dirs(args: APNamespace) -> Tuple[Path, Path, Path, Path, Path | None]:
    ...
    if not config_path.is_absolute():
        config_path = script_dir.parent / 'configs' / config_path

    data_path = script_dir.parent / 'data'
    output_path = script_dir.parent / 'logs'
    checkpoint_path = script_dir.parent / 'checkpoint'
    ...
    return config_path, output_path, data_path, checkpoint_path, resume_path
