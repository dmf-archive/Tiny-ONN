...
class DynamicInfiniteHeadAttention(nn.Module):
    def __init__(self, d_model: int, dtype: torch.dtype = torch.float32):
        ...
        self.sbl_qkv = torch.jit.script(SparseBayesianLinear(d_model, 3 * d_model, dtype=dtype))
        self.sbl_o = torch.jit.script(SparseBayesianLinear(d_model, d_model, dtype=dtype))
...
class DynamicInfiniteExpert(nn.Module):
    def __init__(self, d_model: int, d_ffn_factor: int = 4, dtype: torch.dtype = torch.float32):
        ...
        self.sbl1 = torch.jit.script(SparseBayesianLinear(d_model, d_ffn, dtype=dtype))
        self.sbl2 = torch.jit.script(SparseBayesianLinear(d_ffn, d_model, dtype=dtype))
...
class MoIETransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prior_std: float, kl_epsilon: float) -> tuple[torch.Tensor, list[torch.Tensor], torch.Tensor]:
        attn_in = self.ln1(x)
        attn_out, attn_scores_tuple, attn_masked_tuple, attn_kl = self.attn(attn_in, prior_std, kl_epsilon)
        x = x + self.dropout(attn_out)
        
        ffn_in = self.ln2(x)
        ffn_out, ffn_scores_tuple, ffn_masked_tuple, ffn_kl = self.ffn(ffn_in, prior_std, kl_epsilon)
...
class ReferenceModel(nn.Module):
    def __init__(self):
        ...
        self.blocks = nn.ModuleList([torch.jit.script(MoIETransformerBlock(
