...
class PrunedQwen3DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.layer_idx = layer_idx
        ...
        if self.layer_idx < PRUNE_SELF_ATTN_THRESHOLD:
            self.self_attn = nn.Identity()
        else:
            self.self_attn = Qwen3Attention(config=config, layer_idx=layer_idx)

        if self.layer_idx >= (config.num_hidden_layers // PRUNE_MLP_THRESHOLD_DIVISOR):
...
class Qwen3PreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["PrunedQwen3DecoderLayer"]
...
@auto_docstring
class PrunedQwen3Model(Qwen3PreTrainedModel):
...
@auto_docstring
class PrunedQwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
...
class Qwen3ForQuestionAnswering(Qwen3PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.transformer = PrunedQwen3Model(config)
