...
此阶段的目标是构建一个数字 fMRI 扫描仪，用于精细化数据收集和协同贡献分数 (SPS) 计算。
...
- **目标**: 根据收集到的聚合数据，为**每个块**计算一个量化的协同贡献分数 `SPS`。
- **核心原则**: **高激活 + 低梯度(高稳定性) + 高频次 = 高协同贡献**。
- **已实现**: 已根据激活 Z-score 和梯度范数 Z-score 计算 `SPS`，并已集成到可视化系统中。

## 阶段 0.8: 基于 ∫SPS 扫描结果的残差连接分层剪枝器 [已废弃]

此阶段的目标是根据 PIDiff 扫描结果，对模型进行残差连接分层剪枝，剔除无用的深层 MLP 和浅层自注意力。

### 0.8.1 核心目标：模型瘦身与效率提升 [x]

本阶段的核心任务是利用 PIDiff 扫描获得的协同贡献分数，识别并剪枝 Qwen3-1.7B 模型中贡献度较低的 MLP 和自注意力模块，以实现模型瘦身和推理效率的提升。

### 0.8.2 剪枝策略 [x]

- **剪枝粒度**: 剪枝将针对**层级**进行，即根据 PIDiff 结果决定是否保留某个 Transformer 层的 MLP 或自注意力模块。
- **剪枝标准**: 基于 PIDiff 扫描结果中计算出的 `SPS` 分数，设定一个阈值，低于该阈值的模块将被剪枝。
- **残差连接处理**: 剪枝将考虑残差连接的完整性，确保剪枝后的模型结构仍然有效。

### 0.8.3 剪枝实现 [x]

- **模型修改**: 尝试了功能性剪枝（计算旁路）和物理剪枝（移除模块）两种方式。
- **权重移除**: 尝试了在加载时忽略权重和从 state_dict 中手动移除权重两种方式。
- **验证**: 所有尝试最终都导致模型推理输出乱码或行为异常。

### 0.8.4 结论：此路不通 [x]

经过详尽的调试与复盘（详见 `docs/devlog/phase_0.8_pruning_final_summary.md`），我们得出最终结论：

- **策略失败**：直接在宏观层面（移除或跳过整个层）进行剪枝的策略是不可行的。Transformer 模型内部层与层之间的高度耦合性，使得任何此类粗暴的结构修改都会从根本上破坏模型的完整性，导致其无法正常工作。
- **方向调整**：因此，**Phase-0.8 的探索被正式废弃**。项目将不再尝试层级剪枝，而是严格回归下一阶段的核心技术路线。

## 阶段 1: 知识提取阶段：从巨石中外科手术式分离『概念细胞』

此阶段的目标是识别 Qwen3-1.7B 模型中在 SFT 任务上最活跃、最核心的参数，并将其重组为 Tiny-ONN 的 100 个初始专家模块。

### 1.1 核心目标：发现，而非创造 [x]

本阶段的核心任务是对一个已充分预训练的稠密模型（Qwen3-1.7B）进行一次深入的“神经显影”。目的不是去“训练”或“教育”这个模型，而是要**发现并分离**其内部已经形成的、高效的、高度专业化的知识结构。我们基于一个核心假设：一个成熟的大型语言模型，其内部的“专家”——即那些高协同贡献度的参数集群——已经事实性地完成了功能分化。它们是模型在海量数据冲刷下，自发形成的“概念细胞”。我们的工作就是将这些细胞精确地识别出来，并从庞大的组织中“提取”出来。

### 1.2 聚类分析与专家识别 [ ]

- **目标**: 根据 `SPS` 分数和块的其他特征，识别出功能上高度相关的块集合，形成 100 个“高密度因果网络”或“概念细胞”。
- **待实现**:
  1. **特征向量构建**: 为**每个块**构建一个特征向量。该向量包含：其计算出的量化 `SPS` 分数、其低精度的行为摘要（平均激活、平均梯度、激活频率）、以及**结构位置信息**（例如，层索引、模块类型、矩阵内的块索引等，同样可以被量化为特征）。
  2. **聚类**: 对全部 40 亿个参数的特征向量，应用 K-Means 等聚类算法，目标是将其划分为 **100 个簇**。
  3. **可视化**: 使用 t-SNE 或 UMAP 对聚类结果进行降维和可视化，以直观地验证聚类效果和参数的功能分区。

### 1.3 动态提取与专家组装 [ ]

- **目标**: 根据聚类结果，从 Qwen/Qwen3-1.7B 模型中提取**块级参数**，并组装成 100 个专家模块，总参数量控制在 0.1B 或 0.3B 左右。
- **待实现**:
...
此阶段旨在验证“知识提取”的有效性，并向社区展示 Tiny-ONN 的价值。
...
- **技术报告**: 撰写详细的技术报告，完整记录整个“知识提取”工作流、实验结果和可解释性分析。
