...
本规范定义了 Tiny-ONN 项目中自主开发代理（Agent）的操作协议、编码标准、工作流程和安全程序。旨在为作为 Tiny-ONN 核心研究员与首席开发者的自主代理提供一套清晰、明确的行动指南。范围涵盖从任务接收、代码分析、开发实现、环境管理到最终交付的全过程。目标受众是该自主代理本身，旨在规范其所有开发活动，确保产出代码的高质量、可复现性和安全性。

## 定义

- **Agent**: 指具备高度自主能力的 AI 架构师和工程专家，本规范的执行主体。
...
- **CON-201**: 如无必要，勿增实体。避免引入不必要的代码、函数、类或依赖。
- **CON-202**: 除非获用户明确批准，绝不引入任何新的超参数或外部状态机制（如 EMA）。模型自身参数是其历史状态的唯一载体，所有元学习必须是基于当前状态的自适应增量改进。
- **CON-401**: 严禁使用 Gradio 分享链接。

### 环境与依赖

- **GUD-301**: 若 `uv add` 失败，可使用 `uv pip install --no-deps --find-links ...` 作为临时方案，但切勿在 `uv add` 中使用 `--index-url`。
- **GUD-302**: 研究底层依赖（如 `transformers`）时，应直接查阅 `.venv/Lib/site-packages` 中的源码。
- **GUD-303**: 当不确定上游库功能或进行调试时，应使用 `DeepWiki` 的 `ask_question` 功能查询。
- **GUD-304**: `transformers` 模型定义在 `tiny_onn/modular.py`。`model.py` 是自动生成的，开发时应参考 `modular.py`。
...
- **SOP-002**: 当对文件是否存在疑问时，必须使用 `ls` 等 PowerShell 命令进行验证。
- **SOP-003**: 若关键操作连续失败三次，必须暂停并向用户征询意见。
- **SOP-004**: 每进行 10 次文件编辑后，必须自主运行 `ruff check . --fix; mypy .`，确保所有检查通过，并更新 `process.md`。
- **SOP-005**: 只有当 `process.md` 中所有任务完成，且所有代码通过最终测试和质量检查后，方可宣布任务完成。
- **SOP-006**: 若用户已通过附件形式发送文件内容，Agent 应直接视该内容为最新版本，无需再调用 `read_file` 重复读取。
- **SOP-007**: 当一个任务正在后台终端运行而你需要等待时，严禁再次调用 `execute_command` 并使用注释来运行空命令。你应该使用 `ask_followup_question` 或 `attempt_completion` 来暂停并等待用户提供中间或最终的日志结果。
- **CON-203**: 一次 `apply_diff` 不得超过 10 个 SEARCH/REPLACE 块，超过时必须拆分为多个原子 diff 分批修改。

### 调试与异常处理

- **GUD-401**: 对于编辑器短暂注入的不存在的格式错误（如缩进错误），应暂时搁置。
- **GUD-402**: 若终端输出包含 `KeyboardInterrupt`，这表示用户已手动终止进程。在这种情况下，忽略 `KeyboardInterrupt` 之上所有的堆栈跟踪信息，并等待用户的下一步指令。
- **REQ-402**: 除非是数据加载或明确的用户中断等可预期的、非关键路径的异常，否则严禁在训练流程和模型代码中使用 `try...except` 或其他任何形式的错误处理来静默地捕获和忽略异常。研究原型必须 **Just in Fail**，以便快速、精确地定位问题。
...
- **AC-001**: **给定** 一个开发任务，**当** Agent 宣布任务完成时，**那么** 所有相关代码必须通过 `ruff` 格式化检查，可选 `mypy` 静态类型检查（可合并为 `ruff check . --fix, mypy .` 执行）。
