...
- **证据 (Evidence)**: 指文件系统列表、文件内容、用户明确指令、测试输出等确凿信息，是 Agent 行动的唯一依据。
- **元学习 (Meta-Learning)**: 指模型学习如何学习的过程，在本规范中特指通过计算主损失对中间激活的梯度来优化路由决策的过程。
...
- **GUD-001**: **证据驱动**: 所有行动必须基于确凿证据。在接受任何编码任务时，第一个动作必须是分析现有代码。当假设与证据冲突时，必须立即抛弃假设。
- **GUD-002**: **复用优先**: 在编写任何代码之前，必须首先探查项目中是否存在可复用的组件。绝不重造轮子。
- **GUD-003**: **第一性原理诊断**: 当模型行为与预期严重不符且初步修复无效时，必须暂停试错，回归第一性原理进行形式化分析，避免在原理性缺陷上浪费资源。
...
- **REQ-202**: **类型安全**: 所有新代码必须包含完整且严格的类型标注，尽一切可能避免使用 `Any` 类型，且必须通过 `mypy .` 的静态类型检查。
- **REQ-203**: **张量类型**: 所有张量必须使用 `bfloat16` 类型。
- **REQ-204**: **纯函数**: 所有数据处理和计算逻辑应设计为纯函数，无任何外部副作用。
- **REQ-205**: **并行优化**: 所有计算逻辑必须尽可能利用并行化的张量编程，并在 PyTorch 计算图内完成。
- **CON-201**: **奥卡姆剃刀**: 如无必要，勿增实体。避免引入不必要的代码、函数、类或依赖。
- **CON-202**: **拒绝超参**: 除非获用户明确批准，绝不引入任何新的超参数。必须优先利用模型内部指标实现自适应动态平衡。
- **REQ-206**: **可复现性**: 禁止任何隐式默认行为，所有参数需显式定义。必要时固定随机种子。
- **REQ-207**: **高阶梯度计算**: 在元学习场景下，必须使用 `torch.autograd.grad(..., create_graph=False, retain_graph=True)` 来获取中间梯度张量。

### 环境与依赖

- **REQ-301**: **环境管理**: 必须通过 `uv add` 命令将所有项目依赖项添加到 `pyproject.toml` 文件中。
...
- **REQ-401**: **向后兼容**: 任何代码修改都必须确保向后兼容性。
- **REQ-402**: **测试驱动开发**: 所有新功能或修复都必须附带相应的 `pytest` 测试用例。
...
- **AC-004**: **给定** 任何代码修改，**那么** 该修改必须保持与旧有实验配置和脚本的向后兼容性。
