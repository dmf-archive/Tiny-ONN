...
graph TD
    ...
    subgraph "纯粹模型层 (src/models/)"
        ...
        Model -- "返回 (logits, loss, routing_logits)" --> Trainer
    end
...
- [ ] **纯粹性**: 模型必须是标准的 `nn.Module`，严禁依赖特定的训练器状态或全局变量。
- [ ] **元数据暴露**: 动态路由模型必须通过 `return_dict` 模式暴露 `routing_logits` 或 `routing_info`。
...
- [ ] **职责分离**: 负责计算 `meta_loss` (FARS/SARS) 和路由相关的诊断指标（如专家利用率、熵）。
- [ ] **非侵入式注入**: 通过 `shaper.wrap_model(model)` 注入必要的 hooks，严禁修改模型主干代码。
- [ ] **优化器协同**: 允许访问优化器的 `state_dict`（特别是 Adam 的二阶矩 `v_t`）以实现无量纲的 FARS 成本计算。

### 3.3 训练与优化规范

- [ ] **优化器锁定**: 默认锁定使用 **ARS2-Neo**，充分利用其测地线滑行与平坦度约束能力。
- [ ] **原子重塑**: 训练脚本必须支持 `optimizer.step(closure)` 模式，确保 SAM 扰动逻辑的正确执行。
- [ ] **Just in Fail**: 严禁在训练流程中使用 `try...except` 静默捕获异常。

## 4. 原子化重构路线图 (Module Surgery)

### Phase 1: CAPR 模块手术 (Module Surgery)

- [ ] 从 `arc_dyntrm` 中剥离核心路由模块 `CAPR`。
- [ ] 实现高性能向量化 `CAPR`，彻底消除 Python 原生 `for` 循环。
- [ ] 封装为标准的 `nn.Module`，确保输入输出符合 ARC-2 协议。

### Phase 2: 统一模型库构建 (Standardization)

- [ ] 建立 `src/models/dynsiha/` 统一命名空间。
- [ ] 实现标准的 `FlatDynSIHA` 基类，移除所有任务特定的硬编码逻辑。
- [ ] 确保模型支持标准的 `transformers` 风格调用接口。

### Phase 3: 解耦训练器实现 (Decoupled Trainer)

- [ ] 编写符合 ARC-2 规范的极简 `src/tasks/arc/trainer.py`。
- [ ] 实现任务逻辑（Grid 处理、Loss Masking）与模型架构的彻底分离。
- [ ] 集成 `Shaper` 与 `Observer` 协议，完成闭环。
