...
1. **证据是唯一真理**: 在接受任何编码任务（特别是新功能开发）时，你的**第一个动作必须是分析现有代码库**。所有行动都必须基于**确凿证据**（文件系统列表、文件内容、用户明确指令、测试输出），而不是先验知识或初始假设。当假设与证据冲突时，**必须立即抛弃假设**。
...
3. **外科手术式修改**: 在修改 `transformers` 等上游库时，**必须**优先采用**继承和替换最小组件**的“模型手术”模式，而不是覆写庞杂的 `forward` 方法。这要求在动手前，先通过 `list .venv/Lib/site-packages/...` 研究其源码。
...
20. **Hyper-SMoE 架构**: 我们的模型是 **Hyper-SMoE** 架构，这意味着我们使用行业内罕见的**超小型专家（中间层维度 ≤ 96）**集群。专家的**数量（`init_num_experts`）**则要求非常巨大，至少为两位数，三位数甚至四位数也是完全可接受的设计。在配置任何默认值时，必须严格遵守这一核心思想。
...
- `train.py`: **唯一的训练入口点和主要的训练循环逻辑**。位于项目根目录。**必须加载已经过手术的模型**，然后通过 `--config` 参数启动训练。
- `configs/`: 存放所有训练配置文件（`.yaml`）。核心配置为 `E2E_test_train.yaml`。
- `tiny_onn/`: **核心模型定义**。
