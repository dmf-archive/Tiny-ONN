...
## REQ-004: 核心优化理论：自然梯度 vs. 牛顿法

- **Hessian 矩阵**: 描述**损失函数 `L(θ)`** 的局部几何（曲率）。基于 Hessian 的优化（如牛顿法）旨在寻找损失地形的极小值点。**此方法已被明确禁止**，因为它不关心模型输出的统计特性。
- **Fisher 信息矩阵 (FIM)**: 描述**模型输出分布 `p(y|x, θ)` 的参数化流形**的局部几何。它衡量的是参数 `θ` 的微小变化对模型输出分布 `p(y|x, θ)` 的影响大小（以 KL 散度衡量）。
- **核心方向**: 我们的目标是**自然梯度 (Natural Gradient)**，其更新路径遵循 `p(y|x, θ)` 分布空间的测地线。这保证了在参数空间中移动相同的“距离”，在分布空间中也移动相同的距离，从而避免破坏已有的预测结构。**所有二阶优化方案必须围绕 Fisher 矩阵及其近似展开，严禁重提 Hessian 矩阵。**
