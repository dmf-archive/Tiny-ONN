...
标准 BPTT 通过链式法则耦合所有时间步，导致模型学习到脆弱的、路径依赖的"计算轨迹"。SIA 采取不同策略：

```python
# 标准 BPTT
h_t = h_{t-1} + block(h_{t-1})  # 梯度贯穿全程

# SIA
h_t = h_{t-1} + block(h_{t-1}).detach()  # 切断梯度连接
```

通过 `detach()` 强制每一步独立对目标 `y` 进行预测，梯度叠加时：

- **干涉相消**：每层特有的"独特信息"梯度相互抵消
- **干涉相长**：跨步骤通用的"协同信息"梯度得到增强

最终 `θ_shared` 沉淀下来的是**协同逻辑原子**——所有步骤的"最大公约数"。
...
PLSD 放弃强化学习的试错范式，转向**全量观测的自监督对齐**。

**核心机制**：

1. **生成损失序列**（训练期展开至 T_max）：

   ```
   ℒ_seq = {ℓ₁, ℓ₂, ..., ℓ_T_max}
   ℓₜ = CrossEntropy(LMHead(hₜ), y)
   ```

2. **定义 Oracle 步长**（认知饱和点）：

   ```
   t* = argmin_{t ∈ [1, T_max]} ℓₜ
   ```

   `t*` 代表模型在当前参数下能达到的最佳认知水平。

3. **自监督对齐损失**：

   ```
   Targetₜ = 0 if t < t* else 1
   ℒ_ACT = Σₜ BCE(HaltHead(hₜ), Targetₜ)
   ```

**与 Q-Learning 的对比**：

| 特性 | Q-Learning-ACT | PLSD-ACT |
|:---|:---|:---|
| **监督密度** | 极稀疏（仅停止步） | 极稠密（全路径观测） |
| **优化目标** | 期望奖励最大化 | 认知饱和点对齐 |
| **计算开销** | 较低（单次前向） | 较高（训练需全量展开） |
| **收敛速度** | 慢，需大量探索 | 极快，直接拟合 Loss 地形 |
| **MDL 兼容性** | 弱（需显式惩罚项） | 强（天然寻找最短有效路径） |

### 4.4 为什么 PLSD 是 RDS 的自然选择

**关键洞察**：RDS 的权重绑定递归架构天然支持**逐层解码**。

标准 Transformer 即使权重绑定，也很少在每一层都增加解码头——因为层间参数独立，层解码无意义。但 RDS 将"层"展开为"时间步"，每一步都在**同一个动态 Block** 中执行，通过稀疏路由动态组装了**内容感知的 Logic Layer**。

这意味着：

- 每一步 `hₜ` 都代表当前认知状态的完整表征
- 每一步都可以独立解码，评估当前认知水平
- `t*` 不仅是停止信号，更是**认知饱和的度量**

**PLSD 的"代价"与"收益"**：

- **代价**：训练期需全量展开，激活值和梯度占用更高
- **收益**：
  1. 深度可控，避免无限外推
  2. 符合 MDL 原则，简单任务自动使用浅层
  3. 为推理期自适应早停提供监督信号

**Q-Learning 的保留价值**：

在可能需要**无限外推**的场景（如开放式推理、持续对话），Q-Learning 的贝尔曼自举可能更有意义。但对于 ARC 这类追求最小描述长度的任务，PLSD 是更优选择。
...
其中：
ℒ_main = CrossEntropy(LMHead(h_{t*}), y)  # 主任务损失（Oracle 步解码）
ℒ_FARS = Σₜ Σₑ Beliefₜ,ₑ · Cost_FARS(e)    # 路由塑造损失
ℒ_ACT = Σₜ BCE(HaltHead(hₜ), Targetₜ)      # 深度对齐损失
```
...
```
for t = 1 to T_max:
    hₜ = hₜ₋₁ + DynSIHABlock(hₜ₋₁)
    halt_prob = HaltHead(hₜ)
    if halt_prob > τ:
        return LMHead(hₜ)  # 自适应早停
return LMHead(h_T_max)       # 达到最大深度
```
