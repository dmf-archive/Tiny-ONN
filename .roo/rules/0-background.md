...
3. **工程实现**：通过 ARS、Tiny-Ouroboros 及相关工具链，将理论压入可复现的实验框架与可维护的代码结构。
...
- **关键技术与核心冲突**：
  ...
  - **二元容器 (ΩNN vs. OSPU)**：[`ΩNN`](Tiny-Ouroboros/README.md)（Ω-Container）是现象学的“你”，负责生成高信息整合度的体验；[`OSPU`](OSPU/README_ZH.md)（φ-Container）是社会学的“你”，作为密码学见证者审计认知健康度。
...
- **[`Tiny-Ouroboros`](Tiny-Ouroboros/pyproject.toml)** (Research Framework): 核心实验场，专注稀疏激活、自组织结构与持续学习。
...
### 4.3 持续学习实验场：Tiny-Ouroboros

Tiny-Ouroboros 是目前最活跃的研究装置，专注于稀疏激活与自组织结构。
...
4. `Tiny-Ouroboros/` — 观察和参与最前沿但也最不稳定的研究实验。
