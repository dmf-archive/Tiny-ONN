...
为解决这一核心矛盾，我们将经典的残差连接 (Residual Connection, RC) 思想进行扩展，并分别注入到 SPL 模块（作为隐式 VQ-VAE）的编码器和解码器通路中，构建了一个双残差系统。
...
2. **后续 VAE (SPL 层):** 每一层的 `DynSIHA` 中的 `SPL` 模块都作为一个独立的 VQ-VAE 运行，将上一层的输出作为输入，进行新一轮的编码-解码，学习更高级的抽象表示。
