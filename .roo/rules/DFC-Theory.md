...
> 生命的本质，是用内在的确定性对抗外部的不确定性。因此，我们将不确定性留给 DataLoader，用确定性路由缓冲输入x的每一滴随机性——让 x 自己扮演隐变量 z，让 JSD 博弈替代 KL 先验，从而在稠密计算中孕育出1%的稀疏概念火焰。
...
为解决这一核心矛盾，我们将经典的残差连接 (Residual Connection, RC) 思想进行扩展，并分别注入到 SPL 模块（其在功能上扮演了一个隐式 VQ-VAE 的角色）的编码器和解码器通路中，构建了一个双残差系统。

1. **PRC (Prototype Resident Connection): 编码器通路**
...
2. **MuRC (Mu Residual Connection): 解码器通路**
   MuRC 将残差思想注入到 SPL 的计算核心（即 SPL 的解码器部分 `μ`）中。它将 SPL 模块的功能从学习一个**绝对变换** `y = F(x)`，重构为学习一个**增量修正** `y = x + F(x)`：
   `outputₛₚₗ = xₛₚₗ + Gating(xₛₚₗ) ⋅ μ`
   这为元学习信号提供了关键的**梯度高速公路**。`L_main` 对 `outputₛₚₗ` 的梯度现在可以通过 `xₛₚₗ` 路径，无条件、无衰减地反向传播至 SPL 模块的输入。这确保了无论网络深度如何，每个 SPL 模块的 SARS 机制都能接收到高质量、高信噪比的元学习信号，同时也简化了专家（`μ` 矩阵）的学习任务。

PRC 和 MuRC 并非取代位于 Transformer 块外部的**传统残差连接 (TRC)**。三者功能正交，互为补充，共同构成了一个多层梯度管理体系：

| 机制     | 作用范围 | 保护目标         | 功能                                     |
| :------- | :------- | :--------------- | :--------------------------------------- |
| **TRC**  | 块间     | **主任务梯度**   | 确保主损失在整个深度网络中的有效流动。|
| **PRC**  | 模块内   | **编码器稳定性** | 稳定路由决策的上下文，促进分层抽象。  |
| **MuRC** | 模块内   | **元学习梯度**   | 稳定行动决策的上下文，促进分层抽象。  |

这个三层残差结构是 `Tiny-ONN-ARC` 在维持深度抽象能力的同时，保证其动态自组织机制有效性的关键。
