...
### 3.3. 克服灾难性遗忘：上下文感知路由一致性 (CARC)

尽管 SARS 框架在稳定路由和驱动专家分化方面取得了成功，但一个更深层次的挑战——**灾难性遗忘**——在 ARC 任务的训练中暴露出来。旧有的时间平滑正则化 (`L_meta_stability`) 试图通过惩罚路由分布的剧烈变化来提供稳定性，但其核心假设是错误的：它激励模型对所有输入（无论其内容如何）都采用相似的路由策略。这对于 ARC 任务中几何变换带来的、在逻辑上完全不同的问题实例是致命的，它直接阻止了专家向特定功能的分化，并导致了灾难性的遗忘。

### 5.1. 核心思想：由输入相似性决定路由一致性

为从根本上解决此问题，我们引入了**上下文感知路由一致性 (Context-Aware Routing Consistency, CARC)**。其核心思想是，路由策略的一致性不应是无条件的，而应由**输入的相似性**来动态决定：

`Sim(P(t), P(t-1)) ≈ Sim(x(t), x(t-1))`

其中，`P` 是路由分布，`x` 是对应模块的输入（即 `spl_inputs`）。这个简单的公式优雅地统一了“正交性”和“一致性”这两种看似矛盾的需求：

- **正交性**: 当 `Sim(x(t), x(t-1))` 接近 0 时（即输入非常不同），CARC 鼓励 `Sim(P(t), P(t-1))` 也接近 0，从而促进对不同输入使用正交的路由策略。
- **一致性**: 当 `Sim(x(t), x(t-1))` 接近 1 时（即输入非常相似），CARC 鼓励 `Sim(P(t), P(t-1))` 也接近 1，从而保持对相似输入使用一致的路由策略。

该思想被形式化为一个新的损失项 `L_carc`，它旨在最小化路由分布相似度与输入相似度之间的绝对差异：

`L_carc = | Sim_P - Sim_x.detach() |`

- **`Sim_P`**: 当前步骤与上一步骤的路由 `logits` 之间的**余弦相似度**。
- **`Sim_x`**: 当前步骤与上一步骤的 `spl_input` 之间的**余弦相似度**。
- **`.detach()`**: `Sim_x` 作为目标信号，不应接收梯度，以确保梯度仅流向路由参数，强制 `Sim_P` 去逼近 `Sim_x`。

### 5.3. 实现：全局路由策略指纹 (Global Routing Policy Fingerprint)

实现 CARC 的关键挑战在于，如何为两个不同长度和结构的输入（即任务的不同视角）计算出有意义的、可比较的定长表征。我们的解决方案是计算一个“全局路由策略指纹”。

1. **矩阵构造**: 将模型在一个 `forward` 传递中产生的所有 `routing_logits` 张量沿着最后一个维度（专家维度）拼接，形成一个形状为 `(seq_len, sum_of_experts)` 的组合矩阵。这个矩阵的每一行代表一个令牌在所有 SPL 模块上的完整路由决策。
2. **降维与表征**: 在这个矩阵上执行一次 PCA，将所有样本（令牌）投影到 `k` 个主成分上，然后取其均值，最终得到一个 `k` 维的向量。这个向量就是该任务视角的“全局路由策略指纹”。
