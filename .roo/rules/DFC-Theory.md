...
最终，这三个组件通过一个**收益成本比 (Cost-Benefit Ratio)** 模型组合成最终的效-用（`Goodness`）分数。该模型的核心创新在于**协同收益**项：
...
| 机制     | 作用范围 | 保护目标         | 功能                                     |
| :------- | :------- | :--------------- | :--------------------------------------- |
| **TRC**  | 块间     | **主任务梯度**   | 确保主 CE 流在整个深度网络中的有效流动。 |
| **PRC**  | 模块内   | **编码器稳定性** | 稳定路由决策的上下文，促进分层抽象。     |
| **MuRC** | 模块内   | **元学习梯度**   | 为 SARS 机制提供无损的梯度信号高速公路。 |
