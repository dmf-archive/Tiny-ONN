...
class TinyOnnMoE(Qwen3MLP):
    ...
    def forward(self, hidden_states: torch.Tensor, *, surprise_budget: float = 0.5) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            current_state = flat_hidden_states[None, top_x].reshape(-1, hidden_dim)
...
class TinyOnnDecoderLayer(Qwen3DecoderLayer):
    ...
    def forward(self, hidden_states: torch.Tensor, *args, **kwargs):
        surprise_budget = kwargs.pop("surprise_budget", 0.5)
        ...
        if "surprise_budget" in kwargs:
             hidden_states = self.mlp(hidden_states, surprise_budget=kwargs["surprise_budget"])
        else:
...
class TinyOnnForCausalLM(Qwen3ForCausalLM):
    ...
    def forward(self, *args, **kwargs):
        # Allow passing kwargs down to the model
        return super().forward(*args, **kwargs)
